...
intersection() {
	[[ $# -ne 2 ]] && {
		ferror "needs 2 arguments"
		return 1
	}
	[[ (-f $1 || -p $1) && (-f $2 || -p $2) ]] || {
		ferror "arguments need to be a file"
		return 1
	}
	local filea="$1"
	local fileb="$2"
...
srt_anchor() {
	shopt -s nullglob
	{
		...
		[[ ${#mp4s[@]} -ne 1 ]] &&
			{
				ferror "video not found or more than 1 found"
				return 1
			}
		[[ ${#srts[@]} -ne 1 ]] &&
			{
				ferror "subtitle not found or more than 1 found"
				return 1
			}
		local srt="${srts[0]}"
		local mp4="${mp4s[0]}"
